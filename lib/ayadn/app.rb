...
module Ayadn
  class App < Thor
    ...
    %w{action stream api descriptions endpoints cnx view workers settings post status extend databases fileops logs pinboard set alias errors blacklist scroll authorize switch mark nicerank debug check nowwatching}.each { |r| require_relative "#{r}" }
