	...
	class Endpoints
		...
		def self.unified(options)
			if options[:count]
				...
				@options_list = $config.build_query_options({count: $config.options[:counts][:unified]})
			end
		...
		def self.checkins(options)
			if options[:count]
				...
				@options_list = $config.build_query_options({count: $config.options[:counts][:checkins]})
			end
		...
		def self.global(options)
			if options[:count]
				...
				@options_list = $config.build_query_options({count: $config.options[:counts][:global]})
			end
		...
		def self.trending(options)
			if options[:count]
				...
				@options_list = $config.build_query_options({count: $config.options[:counts][:trending]})
			end
		...
		def self.photos(options)
			if options[:count]
				...
				@options_list = $config.build_query_options({count: $config.options[:counts][:photos]})
			end
		...
		def self.conversations(options)
			if options[:count]
				...
				@options_list = $config.build_query_options({count: $config.options[:counts][:conversations]})
			end
		...
		def self.mentions(username, options)
			if options[:count]
				...
				@options_list = $config.build_query_options({count: $config.options[:counts][:mentions]})
			end
		...
		def self.posts(username, options)
			if options[:count]
				...
				@options_list = $config.build_query_options({count: $config.options[:counts][:posts]})
			end
		...
		def self.whatstarred(username, options)
			if options[:count]
				...
				@options_list = $config.build_query_options({count: $config.options[:counts][:default]})
			end
		...
		def self.convo(post_id, options)
			if options[:count]
				...
				@options_list = $config.build_query_options({count: $config.options[:counts][:convo]})
			end
		...
		def self.search(words, options)
			if options[:count]
				...
				@options_list = $config.build_query_options({count: $config.options[:counts][:search]})
			end
		...
		def self.single_post(post_id)
			@options_list = $config.build_query_options({annotations: 1})
		...
		def self.files_list(options)
			if options[:count]
				...
				@options_list = $config.build_query_options({count: $config.options[:counts][:files]})
			end
